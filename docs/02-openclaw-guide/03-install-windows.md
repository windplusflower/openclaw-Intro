...
| 操作 | 访问目标 | 网络要求 |
|------|----------|----------|
| **安装 Winget/Scoop** | Microsoft/GitHub 服务器 | 🔴 需要外网（首次安装必需） |
| **安装 OpenClaw** | GitHub / npm 仓库 | 🔴 需要外网（首次安装必需） |
| **使用国际 AI 模型** | OpenAI、Anthropic、Google 等 API | 🟡 使用该模型时需要外网 |
| **使用国内 AI 模型** | 阿里云百炼、百度千帆等 | 🟢 国内可直连，无需代理 |

**说明：**
- 🔴 **必须外网**：没有外网无法完成
- 🟡 **可选外网**：使用该服务时需要，不用则不需要
- 🟢 **无需外网**：国内可直接访问
